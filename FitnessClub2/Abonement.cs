...
namespace FitnessClub
{
    class Abonement
    {
        ...
        public static List<Abonement> LoadFromFile(string basePath)
        {
            string path = Path.Combine(basePath, "Abonement.txt");
        ...
        public static void SaveToFile(List<Abonement> list, string basePath)
        {
            string path = Path.Combine(basePath, "Abonement.txt");
            File.WriteAllLines(path, list.Select(a => $"{a.Id};{a.Type};{a.Cost};{a.DurationDays}"));
        }
    }
}
