...
namespace FitnessClub
{
    class Workout
    {
        ...
        public static List<Workout> LoadFromFile(string basePath)
        {
            string path = Path.Combine(basePath, "Workout.txt");
        ...
        public static void SaveToFile(List<Workout> list, string basePath)
        {
            string path = Path.Combine(basePath, "Workout.txt");
            File.WriteAllLines(path, list.Select(w => $"{w.Id};{w.Type};{w.Date:yyyy-MM-dd};{w.TrainerId};{string.Join(",", w.Clients)}"));
        }
    }
}
